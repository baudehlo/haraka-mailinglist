/* session.js - sessions module for node.js
   ...
   Additional API modiftications
    ...
    lookupOrCreate returns a session object.
...
exports.session = function( request, response, callback ){
 ...
 var session; 
  ...
  session = lookupOrCreate(request,{
    lifetime:604800
  });
  ...
  if(!session.data.history) { session.data.history = []; }
	if(request.url.search(/.css|.js|.png|.jpg|.jpeg|.gif|.ico/i) == -1) {
		session.data.history.push(request.url);
		// if(session.data.history.length > 100) session.data.history.pop(); // do we want to track everything
	}

  // as a convention, we will set all non-indentified users to null
